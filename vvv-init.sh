...
if [ ! -d "web/wp/wp-admin" ]; then
	...
	wp core install --url=$WP_URL --title="Hosted Wordpress" --admin_user=admin --admin_password=password
	--admin_email=hosting@errorstudio.co.uk --allow-root --path=public/web/wp
